...
merge_clusters <- function(x = sce, cluster_column = "flowsom", merging_table = merging_table, merging_name = "merging1", levels = NULL){
  
  coldata <- colData(sce) %>% as.data.frame()
  coldata$order_int <- 1:nrow(coldata)
  
  colnames(merging_table) <- c(cluster_column, merging_name)
  coldata <- merge(coldata, merging_table) %>% arrange(order_int) %>% dplyr::select(c(colnames(colData(sce)), all_of(merging_name)))
  
  if(is.null(levels)){
    coldata[,merging_name] <- factor(coldata[,merging_name])
  }else if(!is.null(levels)){
    coldata[,merging_name] <- factor(coldata[,merging_name], levels = levels)
  }
  
  colData(x) <- DataFrame(coldata)
  return(x)
}

#### Differential Analysis ####
plot_DA <- function(x = sce, cluster_column = "primary", sample_id = "sample_id", condition = "condition", comparisons = NULL, method = "t.test", paired = FALSE){
  dtp <- colData(x)[c(cluster_column, condition, sample_id)] %>%
    table() %>% 
    as.data.frame() %>%
    dplyr::rename("counts" = Freq)
  
  
  totdtp <- colData(x)[c(sample_id, condition)] %>%
    table() %>% 
    data.frame() %>% 
    dplyr::rename("totcounts" = Freq)
  
  df <- merge(dtp, totdtp) %>% mutate("Freq" = counts/totcounts*100)
  
  p1 <- ggplot(df, aes(x = get(condition), y = Freq, col = get(condition)))+
...
calculate_DA <- function(x = sce, cluster_column = "primary", sample_id = "sample_id", condition = "condition"){
  dtp <- colData(x)[c(cluster_column, condition, sample_id)] %>%
    table() %>% 
    as.data.frame() %>% 
    dplyr::rename("counts" = Freq)
  ...
  dtp <- merge(dtp, totdtp) %>% mutate("Freq" = counts/totcounts*100) %>% na.omit()
  
  return(dtp)
}

# plot Differential State analysis as boxplot facet wrapped by marker
plot_DS <- function(x = sce, assay = "exprs", markers = state_markers(x), cluster_column = "primary", sample_id = "sample_id", condition = "condition"){
  
  mfidf <- cbind(assay(x, all_of(assay)) %>% t() %>% as.data.frame() %>% .[all_of(markers)], 
                 colData(x) %>% as.data.frame() %>% .[all_of(c(cluster_column, sample_id, condition))])
  mfidfga <- mfidf %>% 
    dplyr::group_by(across(all_of(c(condition, sample_id, cluster_column)))) %>% 
...
calculate_DS <- function(x = sce, assay = "exprs", markers = state_markers(x), cluster_column = "primary", sample_id = "sample_id", condition = "condition"){
...
run_DR <- function(x, 
  ...
  df <- x %>% assay(assay) %>% .[all_of(markers), cs]
...
plot_DR <- function(x = sce, color_by = "pheno", dimred = "UMAP", add_labels = TRUE, assay = "exprs", ncol = NULL, nrow = NULL){
  ...
  p1 <- ggplot(df, aes(x = V1, y = V2))+
    geom_point(aes(col = color_by), size = min(4000/nrow(df), 3))+
    theme_minimal()+
    labs(x = paste(dimred, "1", sep = "_"), y = paste(dimred, "2", sep = "_"))
  
  if(add_labels){
    p1 <- p1 + geom_text_repel(data = clustlabs, aes(x = V1, y = V2, label = color_by), size = 5)
  }
  ...
  if(all(all_of(color_by) %in% rownames(sce))){
...
plot_density <- function(x = sce, dimred = "UMAP", color_by = "condition", xlim = NULL, ylim = NULL, bins = 10, colvec = NULL, contures = TRUE, facet = TRUE){
  ...
  if(facet){
    p1 <- p1 + facet_wrap(all_of(color_by))
  }
  
  if(contures){
    p1 <- p1 + geom_density_2d(aes(col = get(color_by)), na.rm = TRUE, bins = bins)
    if(!is.null(colvec)){
