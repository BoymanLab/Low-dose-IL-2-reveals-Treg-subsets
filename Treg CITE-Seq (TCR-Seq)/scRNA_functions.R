...
clusterplusfeature <- function(x, #a Seurat object
  ...
  if(!is.null(features)){
    if(!is.null(protein.features) & !is.null(rna.features)){
      pf <- x@assays[[assay.adt]][all_of(protein.features),] %>% as.matrix() %>% t()  %>% scale_exprs(margin = 2, q = 0.01)
      rf <- x@assays[[assay.rna]][all_of(rna.features),] %>% as.matrix() %>% t()  %>% scale_exprs(margin = 2, q = 0.01)
      f <- cbind(pf, rf) %>% as.data.frame()
    }else if(!is.null(protein.features) & is.null(rna.features)){
      f <- x@assays[[assay.adt]][all_of(protein.features),] %>% as.matrix() %>% t() %>% scale_exprs(margin = 2, q = 0.01) %>% as.data.frame()
    }else if(!is.null(rna.features) & is.null(protein.features)){
      f <- x@assays[[assay.rna]][all_of(rna.features),] %>% as.matrix() %>% t() %>% scale_exprs(margin = 2, q = 0.01) %>% as.data.frame()
    }
  ...
  clusterplot <- ggplot(u, aes(x = dim1, y = dim2))+
    ...
    theme_bw()+
    scale_color_manual(values = brewer.paired(n = max(3, length(unique(u$cluster_id)))))+
...
plot_volcano <- function(dgeres = dgeres, ylims = c(0,NA), xlims = "free", labels = NULL, suffix = "-RNA", log2FC.thresh = 1.0, pval.adj.thresh = 0.05){
  dgeres <- dgeres %>%
    na.omit() %>%
  ...
  if(!is.null(labels)){
    ...
    dgeres[dgeres$p_val_adj < pval.adj.thresh & abs(dgeres$avg_log2FC) > log2FC.thresh, "delabel"] <- dgeres[dgeres$p_val_adj < pval.adj.thresh & abs(dgeres$avg_log2FC) > log2FC.thresh, "gene_nomen"]
  ...
  dgeres$col <- "NO"
  dgeres$col[dgeres$p_val_adj < pval.adj.thresh & dgeres$avg_log2FC > log2FC.thresh] <- "UP"
  dgeres$col[dgeres$p_val_adj < pval.adj.thresh & dgeres$avg_log2FC < -log2FC.thresh] <- "DOWN"
...
plot_fgsea <- function(dge, gene_set, top_n = NULL, pathways = NULL,
  ...
  for(pw in genesets){
    ...
    if(length(pathway) != length(fgsea_sets[[pw]])){
      print(paste0("Warning: ", signif((1 - length(pathway) / length(fgsea_sets[[pw]]))*100, 2), "% of the genes in the gene set ", pw, " were not found in the DGE."))
    }
