...
clusterplusfeature <- function(x, #a Seurat object
  ...
  u$cluster_id <- x@meta.data[,all_of(cluster_column)]
  
  if(!is.factor(u$cluster_id)){
    u$cluster_id <- u$cluster_id %>% as.factor()
  }
  
  u$cluster_id <- u$cluster_id %>% droplevels()
  ...
  if(!is.null(features)){
    if(!is.null(protein.features) & !is.null(rna.features)){
      protein.features <- x@assays[[assay.adt]][] %>% as.data.frame %>% .[all_of(protein.features),] %>% t() %>% as.data.frame()
      rna.features <- x@assays[[assay.rna]][] %>% as.data.frame %>% .[all_of(rna.features),] %>% t() %>% as.data.frame()
      f <- merge(protein.features %>% as.data.frame(), rna.features %>% as.data.frame(), by = 0) %>% column_to_rownames("Row.names")
    }else if(!is.null(protein.features)){
      f <- x@assays[[assay.adt]][] %>% as.data.frame %>% .[all_of(protein.features),] %>% t() %>% as.data.frame()
    }else if(!is.null(rna.features)){
      f <- x@assays[[assay.rna]][] %>% as.data.frame %>% .[all_of(rna.features),] %>% t() %>% as.data.frame()
    }
  ...
  if(!is.null(features)){
    ...
    if(hml){
      ...
      cluster_colvec <- structure(brewer.paired(n = max(nrow(hmdf),3)), names = rownames(hmdf))
...
plot_DR <- function(x, #a Seurat object
                    ...
                    cll = TRUE #cluster label logical
                    ){ 
  ...
  if(all(color_by %in% c(rownames(x@assays[[assay.rna]]), rownames(x@assays[[assay.adt]])))){
    ...
    p1 <- p1 + 
      geom_point(aes(col = cluster_id), size = min(4000/nrow(u), 3))+
      scale_color_manual(values = brewer.paired(n = max(length(unique(u$cluster_id)),3)))+
      guides(col = guide_legend(override.aes = list(size = 3)))+
    ...
    if(!is.null(split_by)){
      p1 <- p1 + facet_wrap(~split) + theme_classic() + theme(strip.background = element_blank())
    }
    if(cll){
      p1 <- p1 + geom_text_repel(data = clustlabs, aes(x = dim1, y = dim2, label = cluster_id), size = 5)
    }
  }
...
plot_DRcolour <- function(x, #a Seurat object
                    color_by = "seurat_clusters",
                    hml = FALSE, #Heat-map logical: should a heatmap of the clusters be plotted? default is TRUE
                    assay.rna = "integratedRNA", 
                    assay.adt = "integrated.adt",
...
plot_density <- function(x, dim = "UMAP", color_by = "condition", xlim = NULL, ylim = NULL, bins = 10){
  ...
  if(is.null(xlim)){
    xlim <- c(min(u$dim1), max(u$dim1)) * 1.2
  }
  if(is.null(ylim)){
    ylim <- c(min(u$dim2), max(u$dim2)) * 1.2
  }
  ...
  p1 <- ggplot(u, aes(x = dim1, y = dim2))+
    ...
    theme(plot.title = element_text(hjust = 0.5),
          strip.background = element_blank())+
    scale_color_manual(values = brewer.paired(n = max(length(unique(u[,all_of(color_by)])),3)))+
    scale_x_continuous(limits = xlim)+
    scale_y_continuous(limits = ylim)+
    labs(x = paste(dim, "1", sep = "_"), y = paste(dim, "2", sep = "_"), col = color_by)
...
plot_pbfeatures <- function(x, pf = NULL, rf = NULL, 
  ...
  p1 <- ggplot(f, aes(x = gp, y = value))+
    ...
    theme(axis.title.x = element_blank(), strip.background = element_blank(), legend.position = "none")+
    scale_colour_manual(values = colvec)+ #add colors
    scale_fill_manual(values = colvec)+ #add colors
    stat_compare_means(comparisons = list(c("pre-treatment", "post-treatment")), label = "p.adj", method = "t.test", paired = TRUE)+
...
plot_pbfeatures2 <- function(x, pf = NULL, rf = NULL, 
                            assay.rna = "RNA", assay.adt = "Protein", 
                            group.by = "timepoint", paired = FALSE){
  features <- c(pf, rf)
  p <- x@assays[[assay.adt]][] %>% as.data.frame() %>% .[all_of(pf),] %>% t() %>% as.data.frame()
  r <- x@assays[[assay.rna]][] %>% as.data.frame() %>% .[all_of(rf),] %>% t() %>% as.data.frame()
  f <- cbind(p, r)
  ...
  p1 <- ggplot(f, aes(x = gp, y = value))+
    ...
    scale_colour_manual(values = colvec)+ #add colors
    scale_fill_manual(values = colvec)+ #add colors
    stat_compare_means(comparisons = list(c("pre-treatment", "post-treatment")), label = "p.adj", method = "t.test")+
    scale_y_continuous(limits = c(0, NA), expand = expansion(mult = c(0.05, 0.12)))
...
plot_features2 <- function(x, pf = NULL, rf = NULL, 
                          ...
                          write.excel = FALSE, excel.path = "feature_boxplots.xlsx"){
...
plot_features3 <- function(x, protein.features = NULL, rna.features = NULL, 
  ...
  if(write.excel){
    write_xlsx(x = f, path = excel.path)
  }
...
plot_volcano <- function(dgeres = dgeres, ylims = c(0,NA), xlims = "free", labels = NULL, suffix = "-RNA"){
  ...
  if(!is.null(labels)){
    ...
    dgeres[dgeres$p_val_adj < 0.05 & abs(dgeres$avg_log2FC) > 0.5, "delabel"] <- dgeres[dgeres$p_val_adj < 0.05 & abs(dgeres$avg_log2FC) > 0.5, "gene_nomen"]
  ...
  dgeres$col <- "NO"
  dgeres$col[dgeres$p_val_adj < 0.05 & dgeres$avg_log2FC > 0.5] <- "UP"
  dgeres$col[dgeres$p_val_adj < 0.05 & dgeres$avg_log2FC < -0.5] <- "DOWN"
  
  p1 <- ggplot(dgeres, aes(x = avg_log2FC, y = -log10(p_val_adj)))+
    geom_point(aes(col = col, size = avgExprs))+
    scale_color_manual(breaks = c("DOWN", "NO", "UP"), values = c("blue", "grey", "red"))+
    labs(x = "Log2 FC", y = "-log10 FDR", col = "Regulation", size = "Positive Cell Fraction")+
    theme_bw()+
    theme(plot.title = element_text(hjust = 0.5))+
    scale_y_continuous(limits = ylims)
  
  if(!is.null(labels)){
    p1 <- p1 + geom_text_repel(aes(label = delabel), min.segment.length = unit(0.1, 'lines'), max.overlaps = 10000, max.time = 5, max.iter = 100000)
  }else{
    p1 <- p1 + geom_text_repel(aes(label = delabel), min.segment.length = unit(0.1, 'lines'))
  }
...
run_fgsea <- function(dge, gene_set, suffix = NULL, sampleSize = 1000){
  ...
  Markers <- dge %>% 
    na.omit() %>%
    mutate(ranking_metric = -log(p_val+1e-300)*sign(avg_log2FC)) %>%
...
gsea_barplot <- function(fgsea_res, top_n = NULL, p_max = 0.05){
  ...
  ggplot(df, aes(x = pathway, y = -log10(padj)))+
    ...
    geom_hline(yintercept = -log10(0.05), col = "red", linetype = "longdash")+ # vertical line
    theme_classic()+
    coord_flip()+
...
plot_fgsea <- function(dge, gene_set, top_n = NULL, pathways = NULL,
  ...
  plotlist <- list()
  for(pw in genesets){
    pathway <- unname(as.vector(na.omit(match(fgsea_sets[[pw]] %>% unique(), names(statsAdj)))))
